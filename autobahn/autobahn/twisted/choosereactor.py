   ...
   if 'bsd' in sys.platform or sys.platform.startswith('darwin'):
      ...
      if current_reactor != 'KQueueReactor':
         try:
            ...
            log.err("WARNING: Running on *BSD or MacOSX, but cannot install kqueue Twisted reactor ({0}).".format(e))
         else:
            if verbose:
...
def install_reactor(explicitReactor = None, verbose = False):
   ...
   if explicitReactor:
      ...
      log.msg("Trying to install explicitly specified Twisted reactor '%s'" % explicitReactor)
      try:
         installReactor(explicitReactor)
      except Exception as e:
         log.err("Could not install Twisted reactor %s%s" % (explicitReactor, ' ["%s"]' % e if verbose else ''))
         sys.exit(1)
   else:
      ## automatically choose optimal reactor
      ##
      if verbose:
         log.msg("Automatically choosing optimal Twisted reactor")
   ...
   if verbose:
      ...
      log.msg("Running Twisted reactor %s" % qual(reactor.__class__))
