...
class SimpleClientProtocol(AutobahnClientProtocol):
   ...
   def testChainedRpc(self):
      ...
      # Using lambda (this is most general, and preferred) : prints 31
      d2 = self.call("square", 6).addCallback(lambda res: self.call("sub", res, 5)).addCallback(self.show)

      # Using partial (this appends result from previous deferred to end) : prints -31
      d3 = self.call("square", 6).addCallback(partial(self.call, "sub", 5)).addCallback(self.show)
