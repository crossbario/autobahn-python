"""
WAMP RESULT Message SerDes Tests

Tests RESULT message serialization and deserialization:
1. Single-serializer roundtrip correctness

RESULT returns the result of a remote procedure call with optional args/kwargs.
...
def result_test_vector():
    """Load RESULT test vector from wamp-proto"""
    return load_test_vector("singlemessage/basic/result.json")


@pytest.fixture(scope="module")
def result_samples(result_test_vector):
    """Extract serialization samples from RESULT test vector"""
    return [s for s in result_test_vector["samples"] if "serializers" in s]
...
def test_result_serialize_to_bytes(serializer_id, result_samples, create_serializer):
    """
    Test RESULT serialization to bytes.
...
def test_result_roundtrip(serializer_id, result_samples, create_serializer):
    """
    Test RESULT roundtrip: deserialize → serialize → deserialize.
