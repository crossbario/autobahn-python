"""
WAMP INVOCATION Message SerDes Tests

Tests INVOCATION message serialization and deserialization:
1. Single-serializer roundtrip correctness

INVOCATION is sent by Dealer to Callee to invoke a registered procedure.

Uses test vectors from: wamp-proto/testsuite/singlemessage/basic/invocation.json
"""
import pytest
...
def invocation_test_vector():
    """Load INVOCATION test vector from wamp-proto"""
    return load_test_vector("singlemessage/basic/invocation.json")


@pytest.fixture(scope="module")
def invocation_samples(invocation_test_vector):
    """Extract serialization samples from INVOCATION test vector"""
    return [s for s in invocation_test_vector["samples"] if "serializers" in s]
...
def test_invocation_deserialize_from_bytes(serializer_id, invocation_samples, create_serializer):
    """
    Test INVOCATION deserialization from canonical bytes.
    ...
    for sample in invocation_samples:
        ...
        for variant in byte_variants:
            ...
            # Basic validations
            assert isinstance(msg, Invocation)
...
def test_invocation_serialize_to_bytes(serializer_id, invocation_samples, create_serializer):
    """
    Test INVOCATION serialization to bytes.
...
def test_invocation_roundtrip(serializer_id, invocation_samples, create_serializer):
    """
    Test INVOCATION roundtrip: deserialize → serialize → deserialize.
    ...
    for sample in invocation_samples:
        ...
        assert isinstance(msg1, Invocation)
        ...
        assert isinstance(msg2, Invocation)
