"""
WAMP CHALLENGE Message SerDes Tests

Tests CHALLENGE message serialization and deserialization:
1. Single-serializer roundtrip correctness

CHALLENGE is sent by a Router to a Client during authentication.

Uses test vectors from: wamp-proto/testsuite/singlemessage/basic/challenge.json
"""
import pytest
...
def challenge_test_vector():
    """Load CHALLENGE test vector from wamp-proto"""
    return load_test_vector("singlemessage/basic/challenge.json")


@pytest.fixture(scope="module")
def challenge_samples(challenge_test_vector):
    """Extract serialization samples from CHALLENGE test vector"""
    return [s for s in challenge_test_vector["samples"] if "serializers" in s]
...
def test_challenge_deserialize_from_bytes(serializer_id, challenge_samples, create_serializer):
    """
    Test CHALLENGE deserialization from canonical bytes.
    ...
    for sample in challenge_samples:
        ...
        for variant in byte_variants:
            ...
            # Basic validations
            assert isinstance(msg, Challenge)
            assert msg.method == sample["expected_attributes"]["method"]


def test_challenge_serialize_to_bytes(serializer_id, challenge_samples, create_serializer):
    """
    Test CHALLENGE serialization to bytes.
    ...
    for sample in challenge_samples:
        ...
        msg = Challenge(method=attrs["method"])
...
def test_challenge_roundtrip(serializer_id, challenge_samples, create_serializer):
    """
    Test CHALLENGE roundtrip: deserialize → serialize → deserialize.
    ...
    for sample in challenge_samples:
        ...
        assert isinstance(msg1, Challenge)
        ...
        assert isinstance(msg2, Challenge)
