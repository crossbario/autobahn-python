"""
WAMP UNREGISTERED Message SerDes Tests

Tests UNREGISTERED message serialization and deserialization:
1. Single-serializer roundtrip correctness

UNREGISTERED is an acknowledgment message with no Details in basic form,
just [UNREGISTERED, UNREGISTER.Request|id]
...
def unregistered_test_vector():
    """Load UNREGISTERED test vector from wamp-proto"""
    return load_test_vector("singlemessage/basic/unregistered.json")


@pytest.fixture(scope="module")
def unregistered_samples(unregistered_test_vector):
    """Extract serialization samples from UNREGISTERED test vector"""
    return [s for s in unregistered_test_vector["samples"] if "serializers" in s]
...
def test_unregistered_deserialize_from_bytes(serializer_id, unregistered_samples, create_serializer):
    ...
    for sample in unregistered_samples:
        ...
        for variant in byte_variants:
            ...
            # Basic validations
            assert isinstance(msg, Unregistered)
...
def test_unregistered_serialize_to_bytes(serializer_id, unregistered_samples, create_serializer):
    ...
    for sample in unregistered_samples:
        ...
        msg = Unregistered(request=attrs["request_id"])
...
def test_unregistered_roundtrip(serializer_id, unregistered_samples, create_serializer):
    """
    Test UNREGISTERED roundtrip: deserialize → serialize → deserialize.
    ...
    for sample in unregistered_samples:
        ...
        assert isinstance(msg1, Unregistered)
        ...
        assert isinstance(msg2, Unregistered)
