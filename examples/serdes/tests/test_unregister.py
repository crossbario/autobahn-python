"""
WAMP UNREGISTER Message SerDes Tests

Tests UNREGISTER message serialization and deserialization:
1. Single-serializer roundtrip correctness

UNREGISTER removes a procedure endpoint from the dealer.
...
def unregister_test_vector():
    """Load UNREGISTER test vector from wamp-proto"""
    return load_test_vector("singlemessage/basic/unregister.json")


@pytest.fixture(scope="module")
def unregister_samples(unregister_test_vector):
    """Extract serialization samples from UNREGISTER test vector"""
    return [s for s in unregister_test_vector["samples"] if "serializers" in s]
...
def test_unregister_deserialize_from_bytes(
    ...
    for sample in unregister_samples:
        ...
        for variant in byte_variants:
            ...
            # Basic validations
            assert isinstance(msg, Unregister)
...
def test_unregister_roundtrip(serializer_id, unregister_samples, create_serializer):
    """
    Test UNREGISTER roundtrip: deserialize → serialize → deserialize.
    ...
    for sample in unregister_samples:
        ...
        assert isinstance(msg1, Unregister)
        ...
        assert isinstance(msg2, Unregister)
