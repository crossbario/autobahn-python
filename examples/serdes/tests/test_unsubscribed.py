"""
WAMP UNSUBSCRIBED Message SerDes Tests

Tests UNSUBSCRIBED message serialization and deserialization:
1. Single-serializer roundtrip correctness

UNSUBSCRIBED is an acknowledgment message with no Details in basic form,
just [UNSUBSCRIBED, UNSUBSCRIBE.Request|id]
...
def unsubscribed_test_vector():
    """Load UNSUBSCRIBED test vector from wamp-proto"""
    return load_test_vector("singlemessage/basic/unsubscribed.json")


@pytest.fixture(scope="module")
def unsubscribed_samples(unsubscribed_test_vector):
    """Extract serialization samples from UNSUBSCRIBED test vector"""
    return [s for s in unsubscribed_test_vector["samples"] if "serializers" in s]
...
def test_unsubscribed_deserialize_from_bytes(serializer_id, unsubscribed_samples, create_serializer):
    ...
    for sample in unsubscribed_samples:
        ...
        for variant in byte_variants:
            ...
            # Basic validations
            assert isinstance(msg, Unsubscribed)
...
def test_unsubscribed_roundtrip(serializer_id, unsubscribed_samples, create_serializer):
    """
    Test UNSUBSCRIBED roundtrip: deserialize → serialize → deserialize.
    ...
    for sample in unsubscribed_samples:
        ...
        assert isinstance(msg1, Unsubscribed)
        ...
        assert isinstance(msg2, Unsubscribed)
