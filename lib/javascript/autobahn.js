   ...
   self._websocket.onmessage = function(e)
   {
      ...
      if (o[1] in self._calls)
      {
         if (o[0] == ab._MESSAGE_TYPEID_CALL_RESULT) {
            ...
            self._calls[o[1]].reject(o[2], o[3]);
         }
      ...
      else if (o[0] == ab._MESSAGE_TYPEID_EVENT)
      {
         ...
         if (subid in self._subscriptions) {
            d = $.Deferred();
            self._subscriptions[subid].resolve(o[2], d);
            self._subscriptions[subid] = d;
         }
      }
   }
...
ab.Session.prototype.subscribe = function(topicuri) {
   ...
   d = $.Deferred();
   self._subscriptions[self._prefixes.resolveOrPass(topicuri)] = d;
