...
/**
 * Indicates whether browser supports WebSockets
 * (RFC6455 or at least Hybi-10 as needed for Autobahn) or
 * the web-socket-js Flash bridge https://github.com/gimite/web-socket-js
 * is available.
 */
ab.browserSupported = false;

(function () {
   var browser = ab.getBrowser();
   if (browser) {

      if (browser.name === "Chrome" && browser.version[0] >= 14) {
         ab.browserSupported = true;

      } else if (browser.name === "Firefox" && browser.version[0] >= 7) {
         ab.browserSupported = true;

      } else if (browser.name === "MSIE" && browser.version[0] >= 10) {
         ab.browserSupported = true;

      } else if (browser.name === "MSIE" && (browser.version[0] === 8 || browser.version[0] === 9) && window.WEB_SOCKET_SWF_LOCATION) {
         ab.browserSupported = true;
         console.log("MSIE8/9 detected .. using Flash bridge");

      } else if (browser.name === "Safari" && window.WEB_SOCKET_SWF_LOCATION) {
         ab.browserSupported = true;
         window.WEB_SOCKET_FORCE_FLASH = true;
         console.log("Safari detected .. forcing Flash bridge");

      } else {
         // browser not supported / Flash bridge not avaiable!
      }
   } else {
      throw "could not detect browser!";
   }
})();

// Logging message for unsupported browser.
ab.browserNotSupportedMessage = "Your browser does not support WebSockets (RFC6455 or at least Hybi-10 as needed for Autobahn)";

// List of browsers supported by Autobahn .. for UI purposes
ab.supportedBrowsers = ["Google Chrome >=14", "Mozilla Firefox >=7", "Microsoft Internet Explorer >=10"];
