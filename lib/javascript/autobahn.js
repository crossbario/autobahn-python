...
// JS Lint options:
// indentation = 3
// predefined: window, WebSocket, MozWebSocket, console, $
// tolerate dangling _ in identifiers
// tolerate multiple var statements per function
   ...
   self._websocket.onopen = function (e)
   {
      if (self._websocket.protocol !== ab._subprotocol) {
         ...
         if (ab._debug) {
            ...
            console.log("speaking protocol " + self._websocket.protocol);
         }
...
ab.Session.prototype.prefix = function (prefix, uri) {
   ...
   var msg = JSON.stringify([ab._MESSAGE_TYPEID_PREFIX, prefix, uri]);
...
ab.Session.prototype.subscribe = function (topicuri, callback) {
   ...
   var msg = JSON.stringify([ab._MESSAGE_TYPEID_SUBSCRIBE, topicuri]);
   self._websocket.send(msg);
...
ab.Session.prototype.unsubscribe = function (topicuri) {
   ...
   var msg = JSON.stringify([ab._MESSAGE_TYPEID_UNSUBSCRIBE, topicuri]);
   self._websocket.send(msg);
...
ab.Session.prototype.publish = function (topicuri, event, excludeMe) {
   ...
   var msg = JSON.stringify([ab._MESSAGE_TYPEID_PUBLISH, topicuri, event, excludeMe]);
   self._websocket.send(msg);
