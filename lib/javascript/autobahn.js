...
/**
 * Indicates whether browser supports WebSockets (RFC6455).
 */
ab.hasWebSockets = ("WebSocket" in window) || ("MozWebSocket" in window);
...
ab.Session = function (wsuri, onopen, onclose, options) {
   ...
   self._websocket.onopen = function (e)
   {
      ...
      if (self._websocket.protocol !== ab._subprotocol) {
         ...
         if (self._options && self._options.skipSubprotocolCheck) {
            if (ab._debugws) {
               console.group("WS Warning");
               console.info(self._wsuri);
               console.log("Server does not speak WAMP, but check disabled!");
