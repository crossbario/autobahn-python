...
# PyPy only comes in 3.8-7.3.7 for now https://hub.docker.com/_/pypy
FROM ${AUTOBAHN_ARCH}/pypy:3.8-slim
...
# do NOT include autobahn[accelerate] in below! "accelerate" must only be included on CPython
COPY ./.wheels /tmp
RUN find /tmp
